...
const multipleDelete = ()=>{
    ...
    form.post(route('portfolio.multipleDelete'));
}
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr v-for="project in projects.data"
                    ...
                    <td class="w-4 p-4">
                        <div class="flex items-center">
                            <input id="checkbox-table-search-1" type="checkbox"
                               :value="project.id" v-model="form.ids"
                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                            <label for="checkbox-table-search-1" class="sr-only">checkbox</label>
                        </div>
                    </td>
                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        <img class="w-10 h-10 rounded-full" :src="project.image" alt="Jese image" />
                        <div class="ps-3">
                    ...
                    <td class="px-6 py-4">
                        <Link :href="route('portfolio.edit',project.id)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit project</Link>
                    </td>
                </tr>
            </tbody>
        </table>
...
<projectPagination :services="projects" />
